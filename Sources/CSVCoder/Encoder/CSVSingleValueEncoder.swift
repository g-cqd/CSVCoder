//
//  CSVSingleValueEncoder.swift
//  CSVCoder
//
//  Implements single value encoding for CSV fields.
//

import Foundation
import Synchronization

// MARK: - CSVSingleValueEncoder

/// An encoder for single values in CSV fields.
/// nonisolated utility type for encoding
nonisolated struct CSVSingleValueEncoder: Encoder {
    // MARK: Lifecycle

    init(configuration: CSVEncoder.Configuration, codingPath: [CodingKey], storage: CSVEncodingStorage) {
        self.configuration = configuration
        self.codingPath = codingPath
        self.storage = storage
    }

    // MARK: Internal

    let configuration: CSVEncoder.Configuration
    let codingPath: [CodingKey]

    nonisolated var userInfo: [CodingUserInfoKey: Any] { [:] }

    nonisolated func container<Key: CodingKey>(keyedBy type: Key.Type) -> KeyedEncodingContainer<Key> {
        // Return a container that will throw when used
        KeyedEncodingContainer(CSVThrowingKeyedEncodingContainer<Key>(codingPath: codingPath))
    }

    nonisolated func unkeyedContainer() -> UnkeyedEncodingContainer {
        CSVPoisonUnkeyedEncodingContainer(
            error: CSVEncodingError.unsupportedType("Unkeyed containers not supported for single values"),
            codingPath: codingPath,
        )
    }

    nonisolated func singleValueContainer() -> SingleValueEncodingContainer {
        CSVSingleValueEncodingContainer(configuration: configuration, codingPath: codingPath, storage: storage)
    }

    // MARK: Private

    private let storage: CSVEncodingStorage
}

// MARK: - CSVThrowingKeyedEncodingContainer

/// A throwing keyed encoding container for detecting nested types.
nonisolated struct CSVThrowingKeyedEncodingContainer<Key: CodingKey>: KeyedEncodingContainerProtocol {
    let codingPath: [CodingKey]

    /// Single error every protocol-required `encode(_:forKey:)` overload
    /// raises. Hoisted to a constant so the 15 typed entry points stay
    /// one-liners.
    private static var nestedError: CSVEncodingError {
        .unsupportedType("Nested types are not supported. Configure nestedTypeEncodingStrategy to enable.")
    }

    mutating func encodeNil(forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: Bool, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: String, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: Double, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: Float, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: Int, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: Int8, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: Int16, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: Int32, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: Int64, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: UInt, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: UInt8, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: UInt16, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: UInt32, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: UInt64, forKey key: Key) throws { throw Self.nestedError }
    mutating func encode(_ value: some Encodable, forKey key: Key) throws { throw Self.nestedError }

    mutating func nestedContainer<NestedKey: CodingKey>(
        keyedBy _: NestedKey.Type,
        forKey _: Key
    ) -> KeyedEncodingContainer<NestedKey> {
        KeyedEncodingContainer(
            CSVPoisonKeyedEncodingContainer<NestedKey>(
                error: CSVEncodingError.unsupportedType("Nested containers are not supported"),
                codingPath: codingPath,
            )
        )
    }

    mutating func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
        CSVPoisonUnkeyedEncodingContainer(
            error: CSVEncodingError.unsupportedType("Nested containers are not supported"),
            codingPath: codingPath + [key],
        )
    }

    mutating func superEncoder() -> Encoder {
        CSVPoisonEncoder(
            error: CSVEncodingError.unsupportedType("Super encoder is not supported"),
            codingPath: codingPath,
        )
    }

    mutating func superEncoder(forKey key: Key) -> Encoder {
        CSVPoisonEncoder(
            error: CSVEncodingError.unsupportedType("Super encoder is not supported"),
            codingPath: codingPath + [key],
        )
    }
}

// MARK: - CSVSingleValueEncodingContainer

/// A single value container for CSV encoding.
nonisolated struct CSVSingleValueEncodingContainer: SingleValueEncodingContainer {
    // MARK: Lifecycle

    init(configuration: CSVEncoder.Configuration, codingPath: [CodingKey], storage: CSVEncodingStorage) {
        self.configuration = configuration
        self.codingPath = codingPath
        self.storage = storage
    }

    // MARK: Internal

    let configuration: CSVEncoder.Configuration
    let codingPath: [CodingKey]

    mutating func encodeNil() throws {
        storage.setValue("", forKey: currentKey)
    }

    mutating func encode(_ value: Bool) throws {
        let formatted = CSVValueFormatter.formatBool(value, strategy: configuration.boolEncodingStrategy)
        storage.setValue(formatted, forKey: currentKey)
    }

    mutating func encode(_ value: String) throws {
        storage.setValue(value, forKey: currentKey)
    }

    mutating func encode(_ value: Double) throws {
        if value.isNaN || value.isInfinite {
            throw CSVEncodingError.invalidValue("Cannot encode \(value) as CSV field")
        }
        storage.setValue(String(value), forKey: currentKey)
    }

    mutating func encode(_ value: Float) throws {
        if value.isNaN || value.isInfinite {
            throw CSVEncodingError.invalidValue("Cannot encode \(value) as CSV field")
        }
        storage.setValue(String(value), forKey: currentKey)
    }

    mutating func encode(_ value: Int) throws { encodeInteger(value) }
    mutating func encode(_ value: Int8) throws { encodeInteger(value) }
    mutating func encode(_ value: Int16) throws { encodeInteger(value) }
    mutating func encode(_ value: Int32) throws { encodeInteger(value) }
    mutating func encode(_ value: Int64) throws { encodeInteger(value) }
    mutating func encode(_ value: UInt) throws { encodeInteger(value) }
    mutating func encode(_ value: UInt8) throws { encodeInteger(value) }
    mutating func encode(_ value: UInt16) throws { encodeInteger(value) }
    mutating func encode(_ value: UInt32) throws { encodeInteger(value) }
    mutating func encode(_ value: UInt64) throws { encodeInteger(value) }

    mutating func encode(_ value: some Encodable) throws {
        // Handle Date specially
        if let date = value as? Date {
            let encoded = try encodeDate(date)
            storage.setValue(encoded, forKey: currentKey)
            return
        }

        // Handle Decimal specially
        if let decimal = value as? Decimal {
            storage.setValue("\(decimal)", forKey: currentKey)
            return
        }

        // Handle UUID specially
        if let uuid = value as? UUID {
            storage.setValue(uuid.uuidString, forKey: currentKey)
            return
        }

        // Handle URL specially
        if let url = value as? URL {
            storage.setValue(url.absoluteString, forKey: currentKey)
            return
        }

        throw CSVEncodingError.unsupportedType("Cannot encode \(type(of: value)) as single CSV value")
    }

    // MARK: Private

    private let storage: CSVEncodingStorage

    private var currentKey: String {
        codingPath.last?.stringValue ?? ""
    }

    /// Shared write path for every `FixedWidthInteger`. `String(value)` uses
    /// the type's own `LosslessStringConvertible` conformance, identical to
    /// the per-type bodies it replaces.
    private mutating func encodeInteger<T: FixedWidthInteger>(_ value: T) {
        storage.setValue(String(value), forKey: currentKey)
    }

    private func encodeDate(_ date: Date) throws -> String {
        try CSVValueFormatter.formatDate(date, strategy: configuration.dateEncodingStrategy)
    }
}

// MARK: - CSVEncodingStorage

/// Storage for encoded CSV values during a single `encode(to:)` invocation.
///
/// Thread-safe: writes from multiple tasks are serialised through a `Mutex`,
/// and `snapshot()` returns a consistent view of both `keys` and `values`
/// under a single lock acquisition. The storage uses a `Dictionary` for
/// lookup plus a parallel `[String]` to preserve insertion order — this
/// benchmarks measurably faster than `OrderedDictionary` for the
/// "insert N distinct keys then read once" access pattern.
nonisolated final class CSVEncodingStorage: Sendable {
    // MARK: Internal

    func setValue(_ value: String, forKey key: String) {
        state.withLock { state in
            if state.values.updateValue(value, forKey: key) == nil {
                state.orderedKeys.append(key)
            }
        }
    }

    func getValue(forKey key: String) -> String? {
        state.withLock { state in state.values[key] }
    }

    func allKeys() -> [String] {
        state.withLock { state in state.orderedKeys }
    }

    func allValues() -> [String: String] {
        state.withLock { state in state.values }
    }

    /// Returns a consistent snapshot of keys and values under a single lock acquisition.
    func snapshot() -> (keys: [String], values: [String: String]) {
        state.withLock { state in (state.orderedKeys, state.values) }
    }

    func reset() {
        state.withLock { state in
            state.values.removeAll(keepingCapacity: true)
            state.orderedKeys.removeAll(keepingCapacity: true)
        }
    }

    // MARK: Private

    private struct State {
        var values: [String: String] = [:]
        var orderedKeys: [String] = []
    }

    private let state = Mutex(State())
}
